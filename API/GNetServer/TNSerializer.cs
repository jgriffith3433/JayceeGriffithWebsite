...
// In order to make your own classes serializable, nothing is needed. GNetServer is automatically able to serialize all public fields.
...
namespace GNetServer
{
	...
	static public partial class Serialization
	{
		...
		static public Type NameToType (string name)
		{
			...
			if (!mNameToType.TryGetValue(name, out type) || type == null)
			{
				...
				else if (name.StartsWith("TList"))
				{
					...
					{
						Type elemType = NameToType(name.Substring(6, name.Length - 7));
						if (elemType != null) type = typeof(GNetServer.GList<>).MakeGenericType(elemType);
					}
					else Tools.LogError("Malformed type: " + name);
				}
...
#endif
		{
			...
			else if (valueType == typeof(GList<string>) && desiredType.IsArray)
			{
				var elemType = desiredType.GetElementType();

				if (typeof(UnityEngine.Object).IsAssignableFrom(elemType))
				{
					var list = (GList<string>)value;
			...
			else if (valueType == typeof(GList<int>) && desiredType.IsArray)
			{
				var elemType = desiredType.GetElementType();

				if (typeof(UnityEngine.Object).IsAssignableFrom(elemType))
				{
					var list = (GList<int>)value;
					var newList = desiredType.Create(list.size) as IList;
			...
			else if (valueType == typeof(GList<string>) && desiredType.IsGenericType)
			{
				var elemType = desiredType.GetGenericArgument();
			...
			else if (valueType == typeof(GList<int>) && desiredType.IsGenericType)
			{
				var elemType = desiredType.GetGenericArgument();
...
#endif
				// If it's a GNetServer list, serialize all of its elements
				if (obj is TList)
				{
#if REFLECTION_SUPPORT
					if (useReflection)
...
#if REFLECTION_SUPPORT
		static GList<string> mFieldNames = new GList<string>();
		static GList<object> mFieldValues = new GList<object>();
			...
			try
			{
				...
				switch (prefix)
				{
					...
					case 98: // GNetServer.List
					{
						...
						{
#if REFLECTION_SUPPORT
							var arrType = typeof(GNetServer.GList<>).MakeGenericType(type);
