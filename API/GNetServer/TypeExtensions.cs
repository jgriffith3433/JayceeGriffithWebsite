...
{
	/// <summary>
	/// Can be used to mark fields as ignored by GNetServer-based serialization.
	/// </summary>

	[AttributeUsage(AttributeTargets.Field | AttributeTargets.Property | AttributeTargets.Method, AllowMultiple = false)]
	public sealed class IgnoredByGNetServer : Attribute { public IgnoredByGNetServer () { } }
	...
	static public class TypeExtensions
	{
		...
		public class CachedType
		{
			...
			GList<FieldInfo> mFieldDict;
			GList<PropertyInfo> mPropDict;
			Dictionary<BindingFlags, GList<CachedMethod>> mMethods;
			...
			public GList<CachedMethod> GetMethods (BindingFlags flags)
			{
				if (mMethods == null) mMethods = new Dictionary<BindingFlags, GList<CachedMethod>>();

				GList<CachedMethod> list = null;

				if (!mMethods.TryGetValue(flags, out list))
				{
					list = new GList<CachedMethod>();
			...
			public GList<FieldInfo> GetSerializableFields ()
			{
				if (mFieldDict == null)
				{
					mFieldDict = new GList<FieldInfo>();
					...
					for (int i = 0, imax = fields.Length; i < imax; ++i)
					{
						...
						if (field.IsDefined(typeof(IgnoredByGNetServer), true)) continue;
#if !STANDALONE
			...
			public GList<PropertyInfo> GetSerializableProperties ()
			{
				if (mPropDict == null)
				{
					mPropDict = new GList<PropertyInfo>();
					...
					for (int i = 0, imax = props.Length; i < imax; ++i)
					{
						...
						if (prop.IsDefined(typeof(IgnoredByGNetServer), true)) continue;
		...
		static Dictionary<string, Dictionary<Type, GList<ExtesionType>>> mCache =
			new Dictionary<string, Dictionary<Type, GList<ExtesionType>>>();
		...
		{
			Dictionary<Type, GList<ExtesionType>> cachedMethod;

			if (!mCache.TryGetValue(name, out cachedMethod) || cachedMethod == null)
			{
				cachedMethod = new Dictionary<Type, GList<ExtesionType>>();
				mCache.Add(name, cachedMethod);
			}

			GList<ExtesionType> cachedList = null;

			if (!cachedMethod.TryGetValue(type, out cachedList) || cachedList == null)
			{
				cachedList = new GList<ExtesionType>();
		...
		[System.NonSerialized] static GList<CachedType> mCachedTypes = null;
		...
		[System.NonSerialized] static GList<Assembly> mFullAssemblyList = null;
		[System.NonSerialized] static GList<Assembly> mRefinedAssemblyList = null;
		[System.NonSerialized] static int mHash = 0;
		[System.NonSerialized] static bool mHashIsValid = false;

		static void CacheTypes ()
		{
			if (mFullAssemblyList == null) mFullAssemblyList = new GList<Assembly>();
			else mFullAssemblyList.Clear();

			if (mRefinedAssemblyList == null) mRefinedAssemblyList = new GList<Assembly>();
			...
			mCachedTypes = new GList<CachedType>();
		...
		static public GList<CachedType> GetTypes ()
		{
		...
		static public GList<FieldInfo> GetSerializableFields (this Type type)
		{
		...
		static public GList<PropertyInfo> GetSerializableProperties (this Type type)
		{
