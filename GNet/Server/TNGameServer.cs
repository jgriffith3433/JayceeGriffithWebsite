...
namespace GNet
{
    ...
    public class GameServer : FileServer
    {
        ...
        public bool Start(int tcpPort = 0, int udpPort = 0, int srcPort = -1)
        {
            ...
            //        UdpProtocol.defaulGNetworkInterface.AddressFamily == System.Net.Sockets.AddressFamily.InterNetwork)
            //    {
        ...
        protected void SendJoinChannel(NetworkPlayer player, ushort playerLimit, bool persistent, string password, int channelId, string requestedLevelName)
        {
            ...
            if (channelId == -2)
            {
                ...
                for (int i = 0; i < mChannelList.size; ++i)
                {
                    ...
                    if (ch.isOpen && (randomLevel || requestedLevelName.Equals(ch.level)) &&
                        (string.IsNullOrEmpty(ch.password) || (ch.password == password)))
                    {
                        channelId = ch.id;
                        break;
                    }
        ...
        protected bool SetAlias(ServerPlayer player, string s)
        {
            ...
            if (mBan.Contains(s))
            {
                Debug.Log("FAILED a ban check: " + player.id + ", " + s);
                RemovePlayer(player);
                return false;
            }
            else
        ...
        //            GNet.Buffer.ReleaseUnusedMemory();
        //            return true;
        //#endif
        //        }
