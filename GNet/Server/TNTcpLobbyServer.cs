...
//			try
//			{
//				mListener = new TcpListener(GNet.NetworkPlayer.defaultListenerInterface, listenPort);
//				mListener.Start(50);
//				mPort = listenPort;
//			}
//#if STANDALONE
...
//					if (tc.stage == NetworkPlayer.Stage.NotConnected)
//					{
//						RemoveServer(tc);
//						mTcp.RemoveAt(i);
//					}
...
//					if (tc.stage != NetworkPlayer.Stage.Connected) continue;
...
//			if (tc.stage == NetworkPlayer.Stage.Verifying)
//			{
...
//						writer.Write(NetworkPlayer.version);
//						writer.Write(tc.id);
...
//				{
//					if (tc.stage == NetworkPlayer.Stage.WebBrowser)
//					{
...
//							if (p.stage == NetworkPlayer.Stage.Connected)
//							{
...
//				// Skip clients that have not yet verified themselves
//				if (player.stage != NetworkPlayer.Stage.Connected) continue;

//				// Skip server links
//				var lastSendTime = player.Get<long>("lastSend");
//				if (lastSendTime == 0) continue;
//				if (pid == 0 || player.id == pid) { player.SendPacket(buff); ++count; }
//			}
//			return count;
//		}
