	...
	public class TNSyncRigidbody : TNBehaviour
	{
		...
		void FixedUpdate ()
		{
			...
			if (isActive && tno.isMine && !tno.hasBeenDestroyed && TNManager.IsInChannel(tno.channelID))
			{
				...
				if (mWasSleeping || pos != mLastPos || Quaternion.Dot(rot, mLastRot) < 0.99f)
				{
					...
					if (isImportant) tno.Send(1, ForwardType.OthersSaved, pos, rot, vel, mRb.angularVelocity);
					else tno.SendQuickly(1, ForwardType.OthersSaved, pos, rot, vel, mRb.angularVelocity);
				}
		...
		public void Sync ()
		{
			if (isActive && !tno.hasBeenDestroyed && TNManager.IsInChannel(tno.channelID))
			{
				...
				tno.Send(1, ForwardType.OthersSaved, mLastPos, mLastRot, vel, mRb.angularVelocity);
			}
		}
	}
}
