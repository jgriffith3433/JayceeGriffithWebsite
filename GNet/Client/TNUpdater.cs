...
namespace GNet
{
    ...
    public class TNUpdater : MonoBehaviour
    {
        ...
        [System.NonSerialized] GList<IUpdateable> mRemoveUpdateable = new GList<IUpdateable>();
        [System.NonSerialized] GList<ILateUpdateable> mRemoveLate = new GList<ILateUpdateable>();
        [System.NonSerialized] GList<InfrequentEntry> mInfrequent = new GList<InfrequentEntry>();
        [System.NonSerialized] GList<IInfrequentUpdateable> mRemoveInfrequent = new GList<IInfrequentUpdateable>();
...
#endif
            {
                if (mStartable.Count != 0)
                {
                    var m = mStartable.Peek() as MonoBehaviour;
                    if (m != null)
                    {
                        if (m.enabled)
                        {
