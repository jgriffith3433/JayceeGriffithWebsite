...
// In order to make your own classes serializable, nothing is needed. GNet is automatically able to serialize all public fields.
		...
		static public Type NameToType (string name)
		{
			...
			if (!mNameToType.TryGetValue(name, out type) || type == null)
			{
				...
				else if (name.StartsWith("TList"))
				{
					...
					{
						Type elemType = NameToType(name.Substring(6, name.Length - 7));
						if (elemType != null) type = typeof(GNet.GList<>).MakeGenericType(elemType);
					}
					else Tools.LogError("Malformed type: " + name);
				}
...
#endif
				// If it's a GNet list, serialize all of its elements
				if (obj is TList)
				{
#if REFLECTION_SUPPORT
					if (useReflection)
			...
			try
			{
				...
				switch (prefix)
				{
					...
					case 98: // GNet.List
					{
						...
						{
#if REFLECTION_SUPPORT
							var arrType = typeof(GNet.GList<>).MakeGenericType(type);
